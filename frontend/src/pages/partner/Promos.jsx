...
const Promos = () => {
 ...
 { id: 1, code: 'FESTIVAL50', type: 'percentage', value: 50, status: 'active', expiry: '2026-10-30', usageLimit: 100, usedCount: 45 },
 { id: 2, code: 'FLAT150', type: 'flat', value: 150, status: 'expired', expiry: '2026-06-15', usageLimit: 500, usedCount: 500 },
 { id: 3, code: 'BOGO', type: 'bogo', value: null, status: 'active', expiry: '2026-12-31', usageLimit: 200, usedCount: 12 }
 ]);
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 {coupons.map((coupon) => (
 <div key={coupon.id} className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm flex flex-col relative overflow-hidden group">
 
 {/* Background design */}
 <div className={`absolute -right-12 -top-12 w-32 h-32 rounded-full blur-3xl opacity-20 ${
 coupon.status === 'active' ? 'bg-emerald-500' : 'bg-[#f5f6fa]0'
 }`}></div>
 ...
 <button onClick={() => handleDelete(coupon.id)} className="p-2 text-slate-300 hover:text-[#e31837] hover:bg-[#e31837]/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
 ...
 coupon.type === 'percentage' ? 'bg-purple-100 text-purple-600' :
 coupon.type === 'bogo' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'
 }`}>
 {coupon.type === 'percentage' && <Percent className="w-5 h-5" />}
 {coupon.type === 'bogo' && <Gift className="w-5 h-5" />}
 {coupon.type === 'flat' && <Zap className="w-5 h-5" />}
 </div>
 <div>
 ...
 <p className="text-[10px] uppercase font-bold text-slate-400">Valid till {new Date(coupon.expiry).toLocaleDateString()}</p>
 </div>
 </div>
 ...
 <span className="text-slate-900 ">{coupon.usedCount} / {coupon.usageLimit}</span>
 </div>
 <div className="w-full bg-slate-100 rounded-full h-1.5">
 <div className={`h-1.5 rounded-full ${coupon.status === 'active' ? 'bg-[#e31837]' : 'bg-slate-400'}`} style={{ width: `${(coupon.usedCount / coupon.usageLimit) * 100}%` }}></div>
 </div>
 </div>
 <div className="ml-6">
 <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
 coupon.status === 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-gray-200'
 }`}>
 {coupon.status}
 </span>
 </div>
