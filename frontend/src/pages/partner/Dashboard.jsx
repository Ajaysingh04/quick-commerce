...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      console.error(err);
      // Fallback for Quick Commerce Dashboard
      setStats({
        totalOrders: 342,
        totalRevenue: 125000,
        pendingOrders: 18,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          value={`₹${stats?.totalRevenue || 12500}`} 
          trend={15.2} 
          icon={TrendingUp} 
        ...
        <StatCard 
          ...
          value={stats?.readyOrders || 12} 
          icon={Package} 
          colorClass="bg-orange-50 text-orange-600"
        />
        <StatCard 
          title="Orders Delivered" 
          value={stats?.completedOrders || 315} 
          icon={Check} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-black text-slate-800">Weekly Order Volume</h3>
          ...
          <div className="h-64 flex items-end justify-between gap-2 pt-4 border-t border-gray-200">
            {[
              ...
              { label: 'Wed', height: '50%' }, { label: 'Thu', height: '80%' },
              { label: 'Fri', height: '90%' }, { label: 'Sat', height: '100%' }, { label: 'Sun', height: '85%' }
            ].map((bar, i) => (
        ...
        {/* Top Selling Items - Quick Commerce Context */}
        <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-black text-slate-800">Top Moving Items</h3>
            <Package className="w-5 h-5 text-slate-400" />
          </div>
          <div className="space-y-4">
            {[
              { name: 'Amul Taaza Toned Milk', orders: 420, price: 54, img: '/assets/Amul%20Milk.jpg' },
              { name: 'Onion (Pyaz) 1kg', orders: 385, price: 35, img: '/assets/Onion2.jpg' },
              { name: 'Lays India\'s Magic Masala', orders: 310, price: 20, img: '/assets/Lays%20Chips.jpg' },
              { name: 'Farm Fresh Eggs (Pack of 6)', orders: 275, price: 55, img: '/assets/Eggs%20(Pack%20of%206).jpg' }
            ].map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-[#f5f6fa] transition-colors">
                <img src={item.img} alt={item.name} className="w-14 h-14 rounded-xl object-cover bg-white p-1 border border-gray-100" />
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold text-slate-900 truncate">{item.name}</h4>
                  <p className="text-xs text-slate-500">{item.orders} Units Sold</p>
                </div>
